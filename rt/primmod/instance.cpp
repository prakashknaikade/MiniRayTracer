...
void Instance::translate(const Vector& t) {
    ...
    transform = product(translate, transform);
...
void Instance::rotate(const Vector& nnaxis, float angle) {
    ...
    transform = product(total, transform);
    inverse = transform.invert();
}
...
void Instance::scale(float f) {
    ...
    transform = product(scale, transform);
    inverse = transform.invert();
}
...
Intersection Instance::intersect(const Ray& ray, float tmin, float tmax) const {
    ...
    Intersection hit = archetype->intersect(new_ray, tmin, tmax);
    if (hit) {
        return(Intersection(hit.distance, ray, hit.solid, (this->inverse.transpose() * (hit.normalVec)).normalize(), ray.getPoint(hit.distance)));
    }
